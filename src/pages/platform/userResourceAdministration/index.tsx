...
export default function userResourceAdministration(){
  ...
  return (
    <PageWrap>
      <Modal title={type == 'add' ? '新建用户资源' : '编辑用户资源'} visible={isModalOpen} onOk={handleOk} onCancel={() => setIsModalOpen(false)}>
        <Form
          form={form}
          name="basic"
          labelCol={{ span: 6 }}
      ...
      <div className="display-flex align-items-center justify-content-space-between margin-bottom-20">
        ...
        <div>
          <Button type='primary' onClick={() => showModal('add')}>新建用户资源</Button>
        </div>
      </div>
      <Table
        columns={columns}
